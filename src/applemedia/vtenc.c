 ...
 * ## Example pipeline
 * |[
 * gst-launch-1.0 -v videotestsrc ! vtenc_h264 ! qtmux ! filesink
 * location=out.mov
 ...
 * ## Example pipeline
 * |[
 * gst-launch-1.0 -v videotestsrc ! vtenc_h264_hw ! qtmux ! filesink
 * location=out.mov
 * ]| Encode a test video pattern and save it as an MOV file
 *
 */
 ...
 * ## Example pipeline
 * |[
 * gst-launch-1.0 -v videotestsrc ! vtenc_prores ! qtmux ! filesink
location=out.mov
 * ]| Encode a test video pattern and save it as an MOV file
 AVE_CONFIG_H
#include "config.h"
...
static void gst_vtenc_get_property(GObject *obj, guint prop_id, GValue *value,
                                   GParamSpec *pspec);
...
static void gst_vtenc_session_dump_properties(GstVTEnc *self,
                                              VTCompressionSessionRef session);
static void gst_vtenc_session_configure_expected_framerate(
    GstVTEnc *self, VTCompressionSessionRef session, gdouble framerate);
static void gst_vtenc_session_configure_max_keyframe_interval(
    GstVTEnc *self, VTCompressionSessionRef session, gint interval);
...
gst_vtenc_session_configure_property_int(GstVTEnc *self,
                                         VTCompressionSessionRef session,
                                         CFStringRef name, gint value);
static OSStatus
gst_vtenc_session_configure_property_double(GstVTEnc *self,
                                            VTCompressionSessionRef session,
                                            CFStringRef name, gdouble value);
static void gst_vtenc_session_configure_allow_frame_reordering(
    GstVTEnc *self, VTCompressionSessionRef session,
    gboolean allow_frame_reordering);
static void gst_vtenc_session_configure_realtime(
    GstVTEnc *self, VTCompressionSessionRef session, gboolean realtime);
...
static void gst_vtenc_base_init(GstVTEncClass *klass) {
  ...
  gst_element_class_set_metadata(
      element_class, longname, "Codec/Encoder/Video/Hardware", description,
      "Ole André Vadla Ravnås <oleavr@soundrop.com>, Dominik Röttsches "
  ...
  src_caps = gst_caps_new_simple(
      codec_details->mimetype, "width", GST_TYPE_INT_RANGE, min_width,
      max_width, "height", GST_TYPE_INT_RANGE, min_height, max_height,
      "framerate", GST_TYPE_FRACTION_RANGE, min_fps_n, min_fps_d, max_fps_n,
      max_fps_d, NULL);
  ...
  case kCMVideoCodecType_H264:
    gst_structure_set(gst_caps_get_structure(src_caps, 0), "stream-format",
                      G_TYPE_STRING, "avc", "alignment", G_TYPE_STRING, "au",
                      NULL);
    break;
  case kCMVideoCodecType_HEVC:
    gst_structure_set(gst_caps_get_structure(src_caps, 0), "stream-format",
                      G_TYPE_STRING, "hvc1", "alignment", G_TYPE_STRING, "au",
                      NULL);
    break;
  case GST_kCMVideoCodecType_Some_AppleProRes:
    if (g_strcmp0(codec_details->mimetype, "video/x-prores") == 0) {
      ...
      arr = g_value_array_append(arr, &val);
      gst_structure_set_list(gst_caps_get_structure(src_caps, 0), "variant",
                             arr);
...
static void gst_vtenc_class_init(GstVTEncClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_uint(
          "bitrate", "Bitrate", "Target video bitrate in kbps (0 = auto)", 0,
          G_MAXUINT, VTENC_DEFAULT_BITRATE,
          G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property(
      gobject_class, PROP_ALLOW_FRAME_REORDERING,
  ...
  g_object_class_install_property(
      gobject_class, PROP_REALTIME,
      g_param_spec_boolean(
          "realtime", "Realtime", "Configure the encoder for realtime output",
  ...
  g_object_class_install_property(
      ...
      g_param_spec_double(
          "quality", "Quality", "The desired compression quality", 0.0, 1.0,
          VTENC_DEFAULT_QUALITY,
          G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property(
      gobject_class, PROP_MAX_KEYFRAME_INTERVAL,
  ...
  g_object_class_install_property(
      gobject_class, PROP_MAX_KEYFRAME_INTERVAL_DURATION,
      g_param_spec_uint64(
          "max-keyframe-interval-duration", "Max Keyframe Interval Duration",
  ...
  if (g_strcmp0(G_OBJECT_CLASS_NAME(klass), "vtenc_prores") == 0) {
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_PRESERVE_ALPHA,
        g_param_spec_boolean(
            "preserve-alpha", "Preserve Video Alpha Values",
...
static void gst_vtenc_init(GstVTEnc *self) {
  ...
  self->keyframe_props = CFDictionaryCreate(
      NULL, (const void **)keyframe_props_keys,
...
static void gst_vtenc_set_quality(GstVTEnc *self, gdouble quality) {
  GST_OBJECT_LOCK(self);
  self->quality = quality;
  GST_INFO_OBJECT(self, "setting quality %f", quality);
  if (self->session != NULL) {
    gst_vtenc_session_configure_property_double(
        self, self->session, kVTCompressionPropertyKey_Quality, quality);
  }
  GST_OBJECT_UNLOCK(self);
}

static gint gst_vtenc_get_max_keyframe_interval(GstVTEnc *self) {
  gint result;
...
static void gst_vtenc_set_max_keyframe_interval(GstVTEnc *self, gint interval) {
  GST_OBJECT_LOCK(self);
...
gst_vtenc_set_max_keyframe_interval_duration(GstVTEnc *self,
  ...
  if (self->session != NULL) {
    gst_vtenc_session_configure_max_keyframe_interval_duration(
        self, self->session, interval / ((gdouble)GST_SECOND));
  }
  GST_OBJECT_UNLOCK(self);
}

static gboolean gst_vtenc_ensure_output_loop(GstVTEnc *self) {
...
static void gst_vtenc_pause_output_loop(GstVTEnc *self) {
...
static void gst_vtenc_set_flushing_flag(GstVTEnc *self) {
  g_mutex_lock(&self->queue_mutex);
...
static GstFlowReturn gst_vtenc_finish_encoding(GstVTEnc *self,
                                               gboolean is_flushing) {
  GST_DEBUG_OBJECT(self,
                   "complete encoding and clean buffer queue, is flushing %d",
                   is_flushing);
  OSStatus vt_status;
  ...
  vt_status = VTCompressionSessionCompleteFrames(self->session,
...
static gboolean gst_vtenc_h264_parse_profile_level_key(GstVTEnc *self,
                                                       const gchar *profile,
  ...
  self->profile_level = CFStringCreateWithBytes(
      NULL, (const guint8 *)key, strlen(key), kCFStringEncodingASCII, 0);
...
static gboolean gst_vtenc_hevc_parse_profile_level_key(GstVTEnc *self,
                                                       const gchar *profile,
                                                       const gchar *level_arg) {
  gchar *key = NULL;
  ...
  if (profile == NULL || !strcmp(profile, "main"))
    ...
    /* TODO: this should probably be guarded with a version check (macOS 12.3+ /
     * iOS 15.4+)
...
static gboolean gst_vtenc_negotiate_profile_and_level(GstVTEnc *self,
                                                      GstStructure *s) {
...
static gboolean gst_vtenc_negotiate_prores_variant(GstVTEnc *self,
                                                   GstStructure *s) {
    ...
    case GST_kCMVideoCodecType_Some_AppleProRes:
      if (g_strcmp0(self->details->mimetype, "video/x-prores") != 0) {
        GST_ERROR_OBJECT(self,
                         "format_id == %i mimetype must be Apple "
                         "ProRes",
...
static gboolean gst_vtenc_is_negotiated(GstVTEnc *self) {
 ...
 * present.
 */
static gboolean gst_vtenc_signal_ignored_alpha_component(GstVTEnc *self) {
...
static gboolean gst_vtenc_negotiate_downstream(GstVTEnc *self,
                                               CMSampleBufferRef sbuf) {
  gboolean result;
  GstCaps *caps;
  GstStructure *s;
  ...
  gst_structure_set(s, "width", G_TYPE_INT, self->video_info.width, "height",
                    G_TYPE_INT, self->video_info.height, "framerate",
                    GST_TYPE_FRACTION, self->video_info.fps_n,
  ...
  case kCMVideoCodecType_HEVC: {
    ...
    atoms = CMFormatDescriptionGetExtension(
        fmt, kCMFormatDescriptionExtension_SampleDescriptionExtensionAtoms);

    if (self->details->format_id == kCMVideoCodecType_HEVC)
      boxKey = CFStringCreateWithCString(NULL, "hvcC", kCFStringEncodingUTF8);
    else
      boxKey = CFStringCreateWithCString(NULL, "avcC", kCFStringEncodingUTF8);
  ...
  state = gst_video_encoder_set_output_state(GST_VIDEO_ENCODER_CAST(self), caps,
                                             self->input_state);
...
static void gst_vtenc_set_colorimetry(GstVTEnc *self,
                                      VTCompressionSessionRef session) {
  OSStatus status;
  ...
  if (primaries) {
    status = VTSessionSetProperty(
        session, kVTCompressionPropertyKey_ColorPrimaries, primaries);
    GST_DEBUG_OBJECT(self,
  ...
  if (transfer) {
    status = VTSessionSetProperty(
        session, kVTCompressionPropertyKey_TransferFunction, transfer);
    GST_DEBUG_OBJECT(self,
  ...
  if (matrix) {
    status = VTSessionSetProperty(
        session, kVTCompressionPropertyKey_YCbCrMatrix, matrix);
    GST_DEBUG_OBJECT(self, "kVTCompressionPropertyKey_YCbCrMatrix => %d",
                     status);
  }
}

static gboolean gst_vtenc_compute_dts_offset(GstVTEnc *self, gint fps_n,
                                             gint fps_d) {
  ...
  if (fps_d == 0 && num_offset_frames != 0) {
    GST_ERROR_OBJECT(self, "Variable framerate is not supported with B-Frames");
    return FALSE;
  }
...
static VTCompressionSessionRef gst_vtenc_create_session(GstVTEnc *self) {
  ...
  /* Apple's M1 hardware encoding fails when provided with an interlaced ProRes
   * source. It's most likely a bug in VideoToolbox, as no such limitation has
   * been officially mentioned anywhere. For now let's disable HW encoding
   * entirely when such case occurs. */
  gboolean enable_hw =
      !(GST_VIDEO_INFO_IS_INTERLACED(&self->video_info) &&
        codec_details->format_id == GST_kCMVideoCodecType_Some_AppleProRes);

  if (!enable_hw)
    GST_WARNING_OBJECT(
        self,
        "Interlaced content detected, disabling HW-accelerated encoding due to "
        "https://gitlab.freedesktop.org/gstreamer/gstreamer/-/issues/1429");

  encoder_spec =
      CFDictionaryCreateMutable(NULL, 0, &kCFTypeDictionaryKeyCallBacks,
                                &kCFTypeDictionaryValueCallBacks);
  gst_vtutil_dict_set_boolean(
      encoder_spec,
      kVTVideoEncoderSpecification_EnableHardwareAcceleratedVideoEncoder,
      enable_hw);
  ...
  status = VTCompressionSessionCreate(
      NULL, self->video_info.width, self->video_info.height,
  ...
  if (self->profile_level) {
    gst_vtenc_session_configure_expected_framerate(
        self, session,
     ...
     */
    status = VTSessionSetProperty(
        session, kVTCompressionPropertyKey_ProfileLevel, self->profile_level);
    GST_DEBUG_OBJECT(self, "kVTCompressionPropertyKey_ProfileLevel => %d",
                     (int)status);

    status = VTSessionSetProperty(
        session, kVTCompressionPropertyKey_AllowTemporalCompression,
        kCFBooleanTrue);
    GST_DEBUG_OBJECT(self,
                     "kVTCompressionPropertyKey_AllowTemporalCompression => %d",
                     (int)status);

    gst_vtenc_session_configure_max_keyframe_interval(
        self, session, self->max_keyframe_interval);
    gst_vtenc_session_configure_max_keyframe_interval_duration(
        self, session,
  ...
  if (!self->preserve_alpha &&
      (self->specific_format_id == kCMVideoCodecType_AppleProRes4444XQ ||
       self->specific_format_id == kCMVideoCodecType_AppleProRes4444)) {
    status = VTSessionSetProperty(
        session, gstVTCodecPropertyKey_PreserveAlphaChannel, CFSTR("NO"));
  ...
  case GST_VIDEO_INTERLACE_MODE_PROGRESSIVE:
    gst_vtenc_session_configure_property_int(
        self, session, kVTCompressionPropertyKey_FieldCount, 1);
    break;
  case GST_VIDEO_INTERLACE_MODE_INTERLEAVED:
    gst_vtenc_session_configure_property_int(
        self, session, kVTCompressionPropertyKey_FieldCount, 2);
    switch (GST_VIDEO_INFO_FIELD_ORDER(&self->video_info)) {
    case GST_VIDEO_FIELD_ORDER_TOP_FIELD_FIRST:
      status = VTSessionSetProperty(
          session, kVTCompressionPropertyKey_FieldDetail,
  ...
  gst_vtenc_session_configure_allow_frame_reordering(
      self, session, gst_vtenc_get_allow_frame_reordering(self));
  ...
  if (VTCompressionSessionPrepareToEncodeFrames) {
    ...
    if (status != noErr) {
      GST_ERROR_OBJECT(
          self, "VTCompressionSessionPrepareToEncodeFrames() returned: %d",
          (int)status);
    }
  }
#endif
...
static void gst_vtenc_session_dump_property(CFStringRef prop_name,
                                            CFDictionaryRef prop_attrs,
                                            GstVTDumpPropCtx *dpc) {
  gchar *name_str;
  ...
  if (status == noErr) {
    ...
    GST_DEBUG_OBJECT(dpc->self, "%s = <failed to query: %d>", name_str,
                     (int)status);
  }
...
static void gst_vtenc_session_dump_properties(GstVTEnc *self,
  ...
  CFDictionaryApplyFunction(
      dict, (CFDictionaryApplierFunction)gst_vtenc_session_dump_property, &dpc);
  CFRelease(dict);
...
static void gst_vtenc_session_configure_expected_framerate(
    GstVTEnc *self, VTCompressionSessionRef session, gdouble framerate) {
  gst_vtenc_session_configure_property_double(
      self, session, kVTCompressionPropertyKey_ExpectedFrameRate, framerate);
}

static void gst_vtenc_session_configure_max_keyframe_interval(
    GstVTEnc *self, VTCompressionSessionRef session, gint interval) {
  gst_vtenc_session_configure_property_int(
      self, session, kVTCompressionPropertyKey_MaxKeyFrameInterval, interval);
}

static void gst_vtenc_session_configure_max_keyframe_interval_duration(
    GstVTEnc *self, VTCompressionSessionRef session, gdouble duration) {
  gst_vtenc_session_configure_property_double(
      self, session, kVTCompressionPropertyKey_MaxKeyFrameIntervalDuration,
      duration);
}
...
static void gst_vtenc_session_configure_bitrate(GstVTEnc *self,
                                                ...
                                                guint bitrate) {
  gst_vtenc_session_configure_property_int(
      self, session, kVTCompressionPropertyKey_AverageBitRate, bitrate);
}
...
static void gst_vtenc_session_configure_allow_frame_reordering(
  ...
  VTSessionSetProperty(session, kVTCompressionPropertyKey_AllowFrameReordering,
                       allow_frame_reordering ? kCFBooleanTrue
                                              : kCFBooleanFalse);
}
...
static void gst_vtenc_update_latency(GstVTEnc *self) {
  OSStatus status;
  CFNumberRef value;
  int frames = 0;
  ...
  status = VTSessionCopyProperty(
      self->session, kVTCompressionPropertyKey_NumberOfPendingFrames, NULL,
      &value);
  ...
  if (self->latency_frames == -1 || self->latency_frames != frames) {
    ...
    if (self->video_info.fps_d == 0 || self->video_info.fps_n == 0) {
      ...
      frame_duration = gst_util_uint64_scale(GST_SECOND, self->video_info.fps_d,
                                             self->video_info.fps_n);
    }
    latency = frame_duration * frames;
    GST_INFO_OBJECT(
        self, "latency status %d frames %d fps %d/%d time %" GST_TIME_FORMAT,
        status, frames, self->video_info.fps_n, self->video_info.fps_d,
...
static void gst_vtenc_update_timestamps(GstVTEnc *self,
                                        GstVideoCodecFrame *frame,
...
static GstFlowReturn gst_vtenc_encode_frame(GstVTEnc *self,
  ...
  if (task_state == GST_TASK_STOPPED || task_state == GST_TASK_PAUSED) {
    ...
    if (self->downstream_ret != GST_FLOW_OK) {
      if (self->downstream_ret == GST_FLOW_FLUSHING)
        GST_DEBUG_OBJECT(
            self, "Output loop stopped because of flushing, ignoring frame");
      else
        GST_WARNING_OBJECT(self, "Output loop stopped with error (%s), leaving",
                           gst_flow_get_name(self->downstream_ret));
  ...
  if (!self->have_field_order) {
    ...
    if (field_detail) {
      vt_status = VTSessionSetProperty(
          self->session, kVTCompressionPropertyKey_FieldDetail, field_detail);
  ...
  vt_status = VTCompressionSessionEncodeFrame(
      self->session, pbuf, ts, duration, frame_props,
...
static void gst_vtenc_enqueue_buffer(void *outputCallbackRefCon,
  ...
  frame = gst_video_encoder_get_frame(GST_VIDEO_ENCODER_CAST(self),
                                      GPOINTER_TO_INT(sourceFrameRefCon));

  if (status != noErr) {
    if (frame) {
      GST_ELEMENT_ERROR(self, LIBRARY, ENCODE, (NULL),
                        ("Failed to encode frame %d: %d",
...
static gboolean gst_vtenc_buffer_is_keyframe(GstVTEnc *self,
  ...
  if (attachments_for_sample != NULL) {
    ...
    depends_on_others = CFDictionaryGetValue(
        attachments, kCMSampleAttachmentKey_DependsOnOthers);
...
static void gst_vtenc_register(GstPlugin *plugin,
                               const GstVTEncoderDetails *codec_details) {
  GTypeInfo type_info = {
...
#ifndef HAVE_IOS
    {"H.264 (HW only)", "h264_hw", "video/x-h264", kCMVideoCodecType_H264,
     TRUE},
